...
interface StarBorderContainerProps {
    children: ReactNode;
    lightWidth?: number;
    duration?: number;
    lightColor?: string;
...
export function StarBorderContainer({
    children,
    lightWidth = 110,
    duration = 3,
    lightColor = "#FAFAFA",
    ...
    as: Component = "div",
    ...props
}: StarBorderContainerProps) {
    const pathRef = useRef<HTMLDivElement>(null);

    useEffect(() => {
        if (pathRef.current) {
            const div = pathRef.current;
            // Define path based on current size
            const updatePath = () => {
    ...
    return (
        <Component
            ...
            ref={pathRef}
            className={cn(
                "relative z-[40] overflow-hidden inline-flex items-center justify-center transition-colors group/star-button",
                className,
            )}
            {...props}
        >
            ...
            />
            <div
                className="absolute inset-0 dark:border-white/15 border-black/10 z-[4] overflow-hidden rounded-[inherit] pointer-events-none"
            ...
            <div className="z-10 relative w-full h-full flex items-center justify-between">
                {children}
            </div>
        </Component>
    );
