...
export function AdminModal({ editItem, activeTab, fields, handleSave, onClose, data, availableTeams = [], defaultCurrency = '$' }: AdminModalProps) {
    ...
    const defaultValues = {
        ...
        managerId: editItem?.managerId ? String(editItem?.managerId) : '0',
    ...
    // Watch dynamic lookup field
    const lookupValue = form.watch(fields.key)

    // Auto-generate Initials (User)
    useEffect(() => {
        if (!isUserTab) return
        const f = firstName || ''
        const l = lastName || ''
    ...
    }, [firstName, lastName, isUserTab, form])

    // Auto-generate Display (Lookups)
    useEffect(() => {
        if (isUserTab || !lookupValue) return
        const cleanVal = lookupValue.trim()
        if (!cleanVal) return
        ...
        if (isCollision(candidate)) candidate = cleanVal.substring(0, 3).toUpperCase()
        form.setValue('display', candidate, { shouldValidate: true })
    }, [lookupValue, isUserTab, data, editItem, form])
    ...
    const onSubmit = async (data: any) => {
        if (isUserTab) {
            ...
            if ((!hasManager || !hasTeams) && !editItem?.id) {
                ...
                // Determine if we need to warn for edits too - assuming yes based on original logic
                setPendingData(data)
                setConfirmType('missing_info')
                setShowConfirm(true)
                return
    ...
    const handleConfirmProceed = async () => {
        ...
        if (pendingData) {
            ...
            // Ensure name is constructed if needed (duplicate logic from onSubmit, strictly safely)
            if (isUserTab) {
    ...
    useEffect(() => { form.reset(defaultValues) }, [editItem, activeTab])
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-background/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300 slide-in-from-bottom-5">
                <div className="px-8 py-5 flex justify-between items-center bg-muted/40 sticky top-0 backdrop-blur-md z-10 border-b border-white/5">
                    <div>
                        <h3 className="text-xl font-bold text-foreground tracking-tight">{editItem ? 'Edit' : 'Create'} {activeTab}</h3>
                                                    ...
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <div className="space-y-3">
                                                                <Input {...field} placeholder="Paste Image URL..." className="h-9" />
                        ...
                        {!isUserTab ? (
                            <div className="space-y-5">
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{fields.label}</FormLabel>
                                            <FormControl>
                                                <Input {...field} placeholder={`Enter ${fields.label.toLowerCase()}`} className="h-10" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Display Code</FormLabel>
                                            <FormControl>
                                                <Input {...field} placeholder="e.g. Code" className="h-10 font-mono text-sm" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl><Input {...field} className="h-10 font-mono" /></FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    />
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl><Input {...field} placeholder="Role Title" className="h-10" /></FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    />
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        const next = isSelected ? current.filter((id: number) => id !== team.id) : [...current, team.id]
                                                                        field.onChange(next)
                                                                    }}
                                                                >
